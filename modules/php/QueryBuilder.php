...
class QueryBuilder {
  ...
  public function __construct($game, $table, $cast = null, $primary = 'card_id', $log = false) {
    $this->game = $game;
    $this->table = $table;
    $this->cast = $cast;
  ...
  public function run($id = null) {
    ...
    $this->game->DbQuery($this->sql);
    return $this->game->DbAffectedRow();
  }
  ...
  public function get($returnValueIfOnlyOneRow = false, $debug = false) {
    ...
    if ($debug) {
      throw new VisibleSystemException($this->sql);
    }
    $res = $this->game->getObjectListFromDB($this->sql);
    $oRes = [];
    foreach ($res as $row) {
  ...
  public function func($func, $field = null) {
    ...
    $this->assembleQueryClauses();
    return (int) $this->game->getUniqueValueFromDB($this->sql);
  }
  ...
  private function protect($arg) {
    return is_string($arg) ? "'" . $this->game->escapeStringForDB($arg) . "'" : $arg;
  }
